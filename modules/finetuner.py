...
def load_model(model_path,model_arch,arch):
    checkpoint = torch.load(model_path)
    preserved_idx = checkpoint['preserved_idx'] if 'preserved_idx' in checkpoint.keys() else None
    #print(preserved_idx)
    #print(preserved_idx)
    ...
    load(model_arch, state_dict, arch, preserved_idx)
...
class Finetuner:
    def __init__(self, args, logger=None):
        ...
        if self.args.use_kd:
            self.T=7
            #self.alpha=0.3
            #self.soft_criterion=nn.KLDivLoss(reduction="batchmean").cuda()
            #self.teacher_path=r"result/finetuned/vgg_16_bn/KD/148M/model_best.pt"
            self.teacher_path=self.args.teacher_model_path
            if self.args.arch=="vgg_16_bn":
                self.teacher_model=vgg_16_bn().cuda()
            elif self.args.arch=="resnet_32":
                self.teacher_model=resnet_32().cuda()
            ...
            if self.args.KD_mode=="self_prompt":
                if "resnet" in self.args.arch:
                    ...
                    self.prompt_c1=nn.Parameter(torch.randn((64,1,1)),requires_grad=True).cuda()
                    self.prompt_projection1=nn.Sequential(nn.Conv2d(64,64,3,1,1),
                                                         nn.ReLU(),
                                                         nn.Conv2d(64,64,3,1,1)).cuda()

                    #self_prompt_block_2
                    self.prompt_s2=nn.Parameter(torch.randn((64,8,8)),requires_grad=True).cuda()
                    self.prompt_c2 = nn.Parameter(torch.randn((64, 1, 1)), requires_grad=True).cuda()
                    self.prompt_projection2=nn.Sequential(nn.Conv2d(64,64,3,1,1),
                                                         nn.ReLU(),
                                                         nn.Conv2d(64,64,3,1,1)).cuda()

                    #self_prompt_block_3
                    self.prompt_s3=nn.Parameter(torch.randn((64,8,8)),requires_grad=True).cuda()
                    self.prompt_c3 = nn.Parameter(torch.randn((64, 1, 1)), requires_grad=True).cuda()
                    self.prompt_projection3=nn.Sequential(nn.Conv2d(64,64,3,1,1),
                                                         nn.ReLU(),
                                                         nn.Conv2d(64,64,3,1,1)).cuda()
                ...
                if self.args.arch=="vgg_16_bn":
                    ...
                    self.MGD_block=nn.Sequential(nn.Conv2d(in_channels=339,out_channels=339,kernel_size=3,padding=1,stride=1),
                                                 nn.ReLU(),
                                                 nn.Conv2d(in_channels=339,out_channels=339,kernel_size=3,padding=1,stride=1)).cuda()
                elif self.args.arch=="resnet_32":
                    self.algin=nn.Conv2d(in_channels=64,out_channels=64,kernel_size=1,stride=1).cuda()
                    self.MGD_block=nn.Sequential(nn.Conv2d(in_channels=64,out_channels=64,kernel_size=3,padding=1,stride=1),
                                                 nn.ReLU(),
                                                 nn.Conv2d(in_channels=64,out_channels=64,kernel_size=3,padding=1,stride=1)).cuda()
                elif self.args.arch=="resnet_56":
                    self.algin = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=1, stride=1).cuda()
                    self.MGD_block = nn.Sequential(
                        nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, padding=1, stride=1),
                        nn.ReLU(),
                        nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, padding=1, stride=1)).cuda()
                self.loss_mse=nn.MSELoss(reduction="sum").cuda()
            elif self.args.KD_mode=="CAT-KD":
                self.loss_mse=nn.MSELoss(reduction="mean").cuda()
            elif self.args.KD_mode=="MGD+PKD":
                self.loss_mse = nn.MSELoss(reduction="sum").cuda()
                if self.args.arch=="vgg_16_bn":
                    self.algin=nn.Conv2d(in_channels=339,out_channels=339,kernel_size=1,stride=1).cuda()
                    self.MGD_block=nn.Sequential(nn.Conv2d(in_channels=339,out_channels=339,kernel_size=3,padding=1,stride=1),
        ...
        if self.args.dataset == 'imagenet': #ImageNet
            ...
            #要是训练或修剪imagenet数据集的话，就使用CrossEntropyLabelSmooth
            self.criterion = utils.CrossEntropyLabelSmooth(CLASSES, 0.1).cuda()
        else:
            # 要是训练或修剪imagenet数据集的话，就使用CrossEntropyLoss
            self.criterion = nn.CrossEntropyLoss().cuda()
            self.soft_loss=nn.KLDivLoss("reduce=batchmean").cuda()
        self.load_model()
    ...
    def select_index_flops(self, attribution_score, target_flops, r, layer_to_prune):
        ...
        with torch.no_grad():
            ...
            thres = 0.5
            #thres = [0.5]*len(attribution_score)
            delta = 0.25
            eposion=1e-3

            attrib = [[1 if e>thres else 0 for e in l] for l in attribution_score]
            #attrib = [[1 if l[e] > thres[e] else 0 for e in range(len(l))] for l in attribution_score]
            ...
            while abs(flops-target_flops)>50000 and iteration<50:
                ...
                #         thres += delta + eposion  # 两个+eposion 可以得到更高的精度61.799
                #     else:
                #         thres -= (delta + eposion)  # 大于+,小于-,可以得到精度61.569
                #     delta /= 2  # 大于-,小于+,可以得到精度59.5399
                #     # eposion/=2                                                    #两个-,可得到精度59.6899
                #     iteration += 1  # 2回合降低一次eposion的值,可得到61.9899/目前最高可达62.30999755859375
                #     if iteration % 2 == 0:  # 3回合降低一次eposion的值,可得到60.4499
                #         eposion /= 2  # 以上均在cifar10数据集上做的试验
                #     pass
                #二叉搜索,目的是为了让阈值更接近预计FLOPs
                if flops > target_flops: thres += delta+eposion     #两个+eposion 可以得到更高的精度61.799
                else: thres -= (delta+eposion)                      #大于+,小于-,可以得到精度61.569
                delta /= 2                                          #大于-,小于+,可以得到精度59.5399
                #eposion/=2                                                    #两个-,可得到精度59.6899
                iteration +=1                                       #2回合降低一次eposion的值,可得到61.9899/目前最高可达62.30999755859375
                if iteration%2==0:                                  #3回合降低一次eposion的值,可得到60.4499
    ...
    def finetune(self):
        ...
        if self.args.KD_mode is not None:
            if self.args.KD_mode=="self_prompt":
                ...
                optimizer=torch.optim.SGD(list(self.model.parameters())+list(self.MGD_block.parameters()),lr=self.args.lr_finetuning, momentum=self.args.momentum,
                                            weight_decay=self.args.wd, nesterov=self.args.nesterov)
            elif self.args.KD_mode=="MGD+PKD":
                optimizer = torch.optim.SGD(
                    list(self.model.parameters()) + list(self.prompt_projection1.parameters()) + list(
                        self.prompt_projection2.parameters()) + list(self.prompt_projection3.parameters())+list(self.MGD_block.parameters()),
                    lr=self.args.lr_finetuning, momentum=self.args.momentum,
                    weight_decay=self.args.wd, nesterov=self.args.nesterov)
            else:
                optimizer = torch.optim.SGD(self.model.parameters(), lr=self.args.lr_finetuning,
    ...
    def teacher_hook(self,module, input, output):
        # 保存分类器前一个卷积层的特征图
        self.teacher_feature_maps.append(output)
    ...
    def train(self, optimizer, scheduler, total_epochs):
        ...
        def train_one_epoch(epoch, opti, sched):
            ...
            for i, (images, target) in enumerate(self.train_loader):
                ...
                # compute output
                if self.args.KD_mode=="self_prompt" or self.args.KD_mode=="DKD":
                    if "resnet" in self.args.arch:
                        #resnet_32_hook
                        handle=self.model.layer3[-1].shortcut.register_forward_hook(self.hook)
                    elif self.args.arch=="vgg_16_bn":
                        handle = self.model.features.relu16.register_forward_hook(self.hook)
                        #handle = self.model.features.relu12.register_forward_hook(self.hook)
                    elif self.args.arch=="densenet_40":
                        handle=self.model.bn.register_forward_hook(self.hook)
                elif "MGD" in self.args.KD_mode or self.args.KD_mode=="CAT-KD":
                    if self.args.arch=="vgg_16_bn":
                        student_handle=self.model.features.relu16.register_forward_hook(self.hook)
                        teacher_hadndle=self.teacher_model.features.relu16.register_forward_hook(self.teacher_hook)
                    elif self.args.arch=="resnet_32":
                        student_handle=self.model.layer3[-1].shortcut.register_forward_hook(self.hook)
                        teacher_hadndle=self.teacher_model.layer3[-1].shortcut.register_forward_hook(self.teacher_hook)
                    elif self.args.arch=="resnet_56":
                        student_handle = self.model.layer3[-1].shortcut.register_forward_hook(self.hook)
                        teacher_hadndle = self.teacher_model.layer3[-1].shortcut.register_forward_hook(
                            self.teacher_hook)
                ...
                if self.args.KD_mode=="self_prompt" or self.args.KD_mode=="DKD":
                    handle.remove()
                    F=self.feature_maps[0]
                    self.feature_maps[:]=[]
                elif "MGD" in self.args.KD_mode or self.args.KD_mode=="CAT-KD":
                    student_handle.remove()
                    teacher_hadndle.remove()
                    F_student=self.feature_maps[0]
                    #print(F_student.shape)
                    F_teacher=self.teacher_feature_maps[0]
                    self.feature_maps[:]=[]
                    self.teacher_feature_maps[:]=[]
                ...
                if self.args.use_kd:
                   # with torch.no_grad():
                   #     teacher_logits = self.teacher_model(images)
                       #teacher_logits_softmax = torch.nn.functional.softmax(teacher_logits / self.T, dim=1)
                   if self.args.KD_mode=="self_prompt":

                       m1=nn.functional.sigmoid(F*self.prompt_s1)
                       m1=torch.where(m1<0.5,0,1)
                       F_s1=F*m1
                       #F_c1=F_s1*torch.where(nn.functional.sigmoid(self.prompt_c1)<0.5,0,1)
                       F_c1 = F_s1 * nn.functional.sigmoid(self.prompt_c1)
                       F_c1_first=self.prompt_projection1(F_c1)
                       F_c3_second = self.prompt_projection1(F_c1_first)

                       # F_c1_first = self.prompt_projection1(F)
                       # m1 = nn.functional.sigmoid(F_c1_first * self.prompt_s1)
                       # m1 = torch.where(m1 < 0.5, 0, 1)
                       # F_s1 = F_c1_first * m1
                       # # F_c1=F_s1*torch.where(nn.functional.sigmoid(self.prompt_c1)<0.5,0,1)
                       # F_c1 = F_s1 * nn.functional.sigmoid(self.prompt_c1)
                       # F_c1_second = self.prompt_projection1(F_c1)

                       # m2 = nn.functional.sigmoid(F * self.prompt_s2)
                       # m2 = torch.where(m2 < 0.5, 0, 1)
                       # F_s2 = F_c1_second * m2
                       # #F_c2 = F_s2 * torch.where(nn.functional.sigmoid(self.prompt_c2) < 0.5, 0, 1)
                       # F_c2 = F_s2 * nn.functional.sigmoid(self.prompt_c2)
                       # F_c2_first = self.prompt_projection2(F_c2)
                       # F_c2_second = self.prompt_projection2(F_c2_first)

                       # F_c2_first = self.prompt_projection2(F_c1_first)
                       # m2 = nn.functional.sigmoid(F_c1_second * self.prompt_s2)
                       # m2 = torch.where(m2 < 0.5, 0, 1)
                       # F_s2 = F_c1_second * m2
                       # #F_c2 = F_s2 * torch.where(nn.functional.sigmoid(self.prompt_c2) < 0.5, 0, 1)
                       # F_c2 = F_s2 * nn.functional.sigmoid(self.prompt_c2)
                       # F_c2_second = self.prompt_projection2(F_c2)

                       # m3 = nn.functional.sigmoid(F * self.prompt_s3)
                       # m3 = torch.where(m3 < 0.5, 0, 1)
                       # F_s3 = F_c2_second * m3
                       # #F_c3 = F_s3 * torch.where(nn.functional.sigmoid(self.prompt_c3) < 0.5, 0, 1)
                       # F_c3 = F_s3 * nn.functional.sigmoid(self.prompt_c3)
                       # F_c3_first = self.prompt_projection3(F_c3)
                       # F_c3_second = self.prompt_projection3(F_c3_first)

                       # F_c3_first = self.prompt_projection3(F_c3)
                       # F_c3_second = self.prompt_projection3(F_c3_first)

                       # F_c3_first = self.prompt_projection3(F_c2_first)
                       # m3 = nn.functional.sigmoid(F * self.prompt_s3)
                       # m3 = torch.where(m3 < 0.5, 0, 1)
                       # F_s3 = F_c2_second * m3
                       # # F_c3 = F_s3 * torch.where(nn.functional.sigmoid(self.prompt_c3) < 0.5, 0, 1)
                       # F_c3 = F_s3 * nn.functional.sigmoid(self.prompt_c3)
                       # F_c3_second = self.prompt_projection3(F_c3)

                   #print(F_c3_second.shape)
                       if "resnet" in self.args.arch:
                           # F_prompt1=self.model.avgpool(F_c3_first)
                           # F_prompt1=F_prompt1.view(-1,64)
                           # F_prompt1_logits=self.model.linear(F_prompt1)

                           F_prompt=self.model.avgpool(F_c3_second)
                           F_prompt=F_prompt.view(-1,64)
                           F_prompt_logits = self.model.linear(F_prompt)

                       elif self.args.arch=="vgg_16_bn":

                           F_prompt=torch.nn.functional.adaptive_avg_pool2d(F_c3_second,(1,1))
                           #F_prompt = F_prompt.view(-1, 339)
                           F_prompt = F_prompt.view(-1, 339)
                           F_prompt_logits = self.model.classifier(F_prompt)
                       elif self.args.arch=="densenet_40":
                           F_prompt = torch.nn.functional.adaptive_avg_pool2d(F_c3_second, (1, 1))
                           F_prompt = F_prompt.view(-1, 359)
                           F_prompt_logits = self.model.fc(F_prompt)
                       taus = self.args.t_tau * torch.ones_like(teacher_logits)
                       inds = torch.arange(len(logits)).cuda()
                       taus[inds, target] = self.args.tp_tau
                       teacher_logits_softmax = (teacher_logits / taus).softmax(dim=1)
                       kd_loss = self.soft_loss((logits / self.args.s_tau).log_softmax(dim=1), teacher_logits_softmax)

                   elif self.args.KD_mode=="MGD":
                       n,c,h,w=F_student.shape
                       mask=torch.randn((n,c,1,1)).cuda()
                       mask=torch.where(mask<0.5,0,1).cuda()
                       F_mask=torch.mul(F_student,mask)
                       #print(F_mask.shape)
                       F_generation=self.MGD_block(F_mask)
                       kd_loss=self.loss_mse(F_generation,F_teacher)/n
                   elif self.args.KD_mode=="MGD+PKD":
                       n,c,h,w=F_student.shape
                       mask=torch.randn((n,c,1,1)).cuda()
                       mask=torch.where(mask<0.5,0,1).cuda()
                       ...
                       m1 = nn.functional.sigmoid(F_student * self.prompt_s1)
                       m1 = torch.where(m1 < 0.5, 0, 1)
                       F_s1 = F_student * m1
                       # F_c1=F_s1*torch.where(nn.functional.sigmoid(self.prompt_c1)<0.5,0,1)
                       F_c1 = F_s1 * nn.functional.sigmoid(self.prompt_c1)
                       F_c1_first = self.prompt_projection1(F_c1)
                       F_c1_second = self.prompt_projection1(F_c1_first)


                       m2 = nn.functional.sigmoid(F_student * self.prompt_s2)
                       m2 = torch.where(m2 < 0.5, 0, 1)
                       F_s2 = F_c1_second * m2
                       # F_c2 = F_s2 * torch.where(nn.functional.sigmoid(self.prompt_c2) < 0.5, 0, 1)
                       F_c2 = F_s2 * nn.functional.sigmoid(self.prompt_c2)
                       ...
                       m3 = nn.functional.sigmoid(F_student * self.prompt_s3)
                       m3 = torch.where(m3 < 0.5, 0, 1)
                       F_s3 = F_c2_second * m3
                       # F_c3 = F_s3 * torch.where(nn.functional.sigmoid(self.prompt_c3) < 0.5, 0, 1)
                       F_c3 = F_s3 * nn.functional.sigmoid(self.prompt_c3)
                       ...
                       taus[inds, target] = self.args.tp_tau
                       teacher_logits_softmax = (teacher_logits / taus).softmax(dim=1)
                       kd_loss = self.soft_loss((logits / self.args.s_tau).log_softmax(dim=1), teacher_logits_softmax)+self.loss_mse(F_generation,F_teacher)/n
                   elif self.args.KD_mode=="Logit_Stand_KD":
                       student_mean=logits.mean(dim=-1,keepdims=True)
                       student_stdv=logits.std(dim=-1,keepdims=True)
                       teacher_mean=teacher_logits.mean(dim=-1,keepdims=True)
                       teacher_stdv=logits.std(dim=-1,keepdims=True)

                       logits=(logits-student_mean)/(1e-7+student_stdv)
                       teacher_logits=(teacher_logits-teacher_mean)/(1e-7+teacher_stdv)

                       log_logist=(logits/4).log_softmax(dim=1)
                       log_teacher_logist=(teacher_logits/4).softmax(dim=1)

                       #log_logist_softmax=logits.softmax(dim=1)
                       kd_loss=self.soft_loss(log_logist,log_teacher_logist)
                       kd_loss*=4**2
                   elif self.args.KD_mode=="No_self_prompt":
                       taus = self.args.t_tau * torch.ones_like(teacher_logits)
                       inds = torch.arange(len(logits)).cuda()
                       taus[inds, target] = self.args.tp_tau
                       teacher_logits_softmax = (teacher_logits / taus).softmax(dim=1)
                       ...
                       #n,c,h,w=F_teacher.shape
                       #threshold=torch.norm(F_teacher,dim=(2,3),keepdim=True,p=1)/(h*w)
                       #F_teacher=F_teacher-threshold
                       #F_teacher=torch.relu(F_teacher).bool()*torch.ones_like(F_teacher)
                       cam_student=torch.nn.functional.adaptive_avg_pool2d(F_student,(1,1))
                       cam_teacher = torch.nn.functional.adaptive_avg_pool2d(F_teacher,(1,1))
                       kd_loss=self.loss_mse(nn.functional.normalize(cam_student),nn.functional.normalize(cam_teacher))
                   elif self.args.KD_mode=="DKD":

                       gt_mask=self._get_gt_mask(logits,target)
                       other_mask=self._get_other_mask(logits,target)

                       logist_softamx=nn.functional.softmax(logits,dim=1)
                       teacher_logits_softmax=nn.functional.softmax(teacher_logits,dim=1)

                       logist_softamx=self.cat_mask(logist_softamx,gt_mask,other_mask)
                       teacher_logits_softmax=self.cat_mask(teacher_logits_softmax,gt_mask,other_mask)

                       log_logist=torch.log(logist_softamx)
                       kd_loss_part1=self.soft_loss(log_logist,teacher_logits_softmax)*self.T**2

                       kd_loss_part2=self.soft_loss(nn.functional.log_softmax(logits/self.T-1000.0*gt_mask,dim=1),nn.functional.softmax(teacher_logits/self.T-1000.0*gt_mask,dim=1))*self.T**2
                       #print(0.01*kd_loss_part1)
                       #print(8*kd_loss_part2)
                       kd_loss=0.01*kd_loss_part1+kd_loss_part2
                       #print(kd_loss)
                   #kd_loss=self.soft_loss((logits/self.T).log_softmax(dim=1),teacher_logits_softmax)
                   #kd_loss=self.args.s_tau*self.args.s_tau*(-1.0*teacher_logits_softmax*(logits/self.args.s_tau).log_softmax(dim=1)).sum(dim=1).mean()
                   # loss=(1.0-self.args.lamba)*self.criterion(logits,target)+self.args.lamba*kd_loss

                   loss_ce=self.criterion(logits,target)

                   #loss_ce_prompt=self.criterion(F_prompt_logits,target)
                   if self.args.KD_mode=="self_prompt":
                       # loss_prompt = self.args.s_tau * self.args.s_tau * (
                       #             -1.0 * teacher_logits_softmax * (F_prompt_logits / self.args.s_tau).log_softmax(
                       #         dim=1)).sum(dim=1).mean()
                       loss_prompt=self.soft_loss((F_prompt_logits / self.args.s_tau).log_softmax(
                               dim=1),teacher_logits_softmax)
                       # loss_prompt2 = self.args.s_tau * self.args.s_tau * (
                       #         -1.0 * teacher_logits_softmax * (F_prompt1_logits / self.args.s_tau).log_softmax(
                       #     dim=1)).sum(dim=1).mean()

                       #loss = self.args.lamba * loss_ce + (1 - self.args.lamba) * (kd_loss + loss_prompt +loss_prompt2)
                       loss = self.args.lamba * loss_ce + (1 - self.args.lamba) * (kd_loss + loss_prompt)
                   # loss_dkd=dkd_loss(logits,teacher_logits,target,1,8,self.T)
                   #loss=self.args.lamba*loss_ce+(1-self.args.lamba)*kd_loss
                   elif "MGD" in self.args.KD_mode:
                       loss=loss_ce+7e-5*kd_loss
                   elif self.args.KD_mode=="Logit_Stand_KD":
                       loss=0.8*loss_ce+0.2*kd_loss
                   elif self.args.KD_mode=="CAT-KD":
                       loss=self.args.lamba*loss_ce+(1-self.args.lamba)*kd_loss
                   elif self.args.KD_mode=="DKD":
                       loss=self.args.lamba*loss_ce+(1-self.args.lamba)*kd_loss
        ...
        while epoch < total_epochs:
            ...
            #valid_obj, valid_top1_acc, valid_top5_acc = self.validate_KD(self.val_loader, self.model,self.teacher_model, self.criterion,self.soft_criterion,self.alpha)
            if self.args.save_plot:
    ...
    def update_results(self, key, description):
        ...
        new_res['accuracy'] = self.validate(self.val_loader, self.model, self.criterion, mute=True)[1]
        #new_res['accuracy']=self.validate_KD(self.val_loader,self.model,self.teacher_model,self.criterion,self.soft_criterion,self.alpha,mute=True)[1]
        # check the accuracy before decomposition
        self.res[key] = [description, new_res]
