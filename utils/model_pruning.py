...
from utils.arch_modif import prune_layer, get_module_in_model,prune_l0_layer

def prune(model, preserved_indexes_all, layers_to_prune, arch):
    if 'vgg_16' in arch:
        prune_vgg(model, preserved_indexes_all, layers_to_prune)
    elif "l0_vgg_16" in arch:
        prune_l0_vgg(model, preserved_indexes_all, layers_to_prune)
    elif arch == 'resnet_50':
...
# --------------------------------------------------------------------------------
def prune_l0_vgg(model, preserved_indexes_all, layers_to_prune):
    """
    Method to prune vgg.
    """
    previous_pruned = False
    cnt = 0
    for name, module in model.named_modules():
        if isinstance(module, (L0Conv2d, L0Dense)):
            if previous_pruned:
                # prune input channel
                prune_l0_layer(model, name, preserved_indexes, 1)
                previous_pruned = False
            if cnt in layers_to_prune:
                idx = layers_to_prune.index(cnt)
                preserved_indexes = preserved_indexes_all[idx]
                prune_ratio = prune_l0_layer(model, name, preserved_indexes, 0)
                print(f'[{cnt}] Target: {name} ({int(prune_ratio*100)}%)')
                previous_pruned = True
            cnt+=1
        elif isinstance(module, (nn.BatchNorm1d, nn.BatchNorm2d)) and previous_pruned:
            prune_l0_layer(model, name, preserved_indexes)
# --------------------------------------------------------------------------------
