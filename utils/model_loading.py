...
def load_l0_vgg(model:nn.Module, state_dict: dict):
    """
    Method to load vgg.
    Args:
        - model: original model (non pruned)
    ...
    """
    prev_state_dict = model.state_dict()
    #print("prev_state_dict:",prev_state_dict)
    previous_pruned = False
    cnt = 0
    for name, module in model.named_modules():
        if isinstance(module, (L0Conv2d, L0Dense)):
            ...
            if cout != prev_cout:
                preserved_indexes = list(range(cout))
                prune_l0_layer(model, name, preserved_indexes, 0)
                previous_pruned = True
            cnt+=1
        elif isinstance(module, (nn.BatchNorm1d, nn.BatchNorm2d)) and previous_pruned:
